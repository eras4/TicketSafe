...
struct Seat {
    ...
    constructor(address _etherAddress, address _usdtAddress, address _croAddress, address _poapAddress, address _authenticityCertificateAddress) ERC721("TicketSafe", "TS") {
        etherToken = IERC20(_etherAddress);
        usdtToken = IERC20(_usdtAddress);
        croToken = IERC20(_croAddress);
...
function buyTicketERC20(uint256 experienceId, uint256 _ticketId, uint256 _seatTypeId, uint _row, uint _column, IERC20 token) external whenNotPaused {
    ...
    if (requestedExperience.isExperiencePOAP) {
        ...
        // Actualizar los saldos de ether, usdt Y cro del evento
        if (address(token) == address(etherToken)) {
         ...
         } else if (address(token) == address(croToken)) {
        experienceRevenue[experienceId].croRevenue += price;
    }
}
}
...
uint256 croBalance = revenue.croRevenue;
uint256 commissionAmountEther = 0;
uint256 commissionAmountUsdt = 0;
uint256 commissionAmountCro = 0;
...
if (croBalance > 0) {
    commissionAmountCro = croBalance * commission / 100;
    croToken.transfer(owner(), croBalance - commissionAmountCro);
    revenue.croRevenue = commissionAmountCro;
}
